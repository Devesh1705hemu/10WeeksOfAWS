...
## Some of myUnderstandings

1. **Organization Root vs AWS Root User**

   * **Organization Root:** The top-level container in **AWS Organizations**. It contains accounts and organizational units (OUs).
   * **AWS Root User:** The special, highest-privilege identity of an individual AWS account, used for account-level tasks.

2. **Does attaching an SCP allow a user to call an AWS API?**

   * **No.** An **SCP (Service Control Policy)** only sets the maximum permissions available to accounts. It does **not grant permissions** by itself.
   * The user also needs an **IAM/permission-set policy** that allows the action.

3. **Permission Set allows an action, but SCP denies it**

   * The action is **denied**.
   * An explicit **Deny in an SCP overrides an Allow** from a permission set or IAM policy.

4. **Why keep normal workloads out of the Management Account?**

   * The management account has powerful organization-level permissions.
   * Keeping workloads in separate accounts reduces the **blast radius**, improves security isolation, and makes billing/access management easier.

5. **Role name indicating an Identity Center session**

   * Typically: **`AWSReservedSSO_<PermissionSetName>_<unique-id>`**
   * `AWSReservedSSO` indicates the role was created for an **AWS IAM Identity Center** permission set.
   * **Permission Set Name:** The permission set assigned to the user.
   * **Unique ID:** Identifies the specific role instance.
   * Example: `AWSReservedSSO_AdministratorAccess_abc123`

**Key rule to remember:**
**SCP = maximum boundary, not permission grant.**
**IAM/Permission Set = grants permission.**
**Explicit Deny = wins.**
